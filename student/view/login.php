...
if($_SERVER["REQUEST_METHOD"]=="POST")
{
    ...
    if($result->num_rows >0)
    {
        ...
        if(password_verify($password,$row['password']))///password_verify check kore hashed paassword ke
        {
            ...
            {
                header("Location:teacher.php");//link ke redirect kore day///MARUF tomar main dashboard er link dibe ei khane
                exit();
            }
            elseif(preg_match("/^[0-9]{2}-[0-9]{5}-[0-9]{1}$/",$username))
            {
                header("Location:logout.html");///SHEUN tomar main  dashboard er link dibe eikhane
                exit();
